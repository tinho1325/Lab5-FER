  ...
  return (
    <div>
      <div className="page-banner">
        <h1>📰 Food News</h1>
        <p>The latest stories, trends, and events from the culinary world.</p>
      </div>

      <Container className="pb-5">
        <Row>
          {newLists.map(news => (
            <Col key={news.id} xs={12} sm={6} lg={4} xl={3} className="mb-4">
              <Card className="news-card">
                <Card.Img variant="top" src={news.images} alt={news.title} />
                <Card.Body>
                  <Card.Title className="card-title">{news.title}</Card.Title>
                  <Card.Text className="text-muted" style={{ fontSize: '0.88rem' }}>
                    {news.description}
                  </Card.Text>
                  <a href="#!" className="btn btn-sm btn-orange mt-auto">Read More</a>
                </Card.Body>
              </Card>
            </Col>
          ))}
